...
class DQNAgent(object):
    ...
    def train(self, verbose=False):
        ...
        while iteration < self.iterations: # for the set number of iterations
            ...
            if self.iteration % self.print_every == 0 and verbose:
                print("Iteration: " +  self.iteration, ", Loss: ", loss, " Q-Values: ", qvals, ", Time since print: ",
                        timer() - last_print, ", Total runtime: ", timer() - start_time)
                last_print = timer()
            ...
            if self.iteration % self.eval_every == 0: # evaluation
                ...
                if verbose:
                    print("Evaluation, total reward: ", rtot, ", Total runtime: ", sim_time)
    ...
    def batch_update(self):
        ...
        loss, qvals = self.learner.update(oahist, a, r, oaphist)
        return loss, qvals
...
def set_params(self, settings):
        ...
        self.ohist = np.zeros((self.ohist_size,) + simulator.model_dims, dtype=np.float32)
        self.ahist = np.zeros(self.ahist_size, dtype=np.int32)
        self.rev_ohist = np.zeros((self.ohist_size,) + simulator.model_dims, dtype=np.float32)
        self.rev_ahist = np.zeros(self.ahist_size, dtype=np.int32)

        self.eval_ohist = np.zeros((self.ohist_size,) + simulator.model_dims, dtype=np.float32)
        self.eval_ahist = np.zeros(self.ahist_size, dtype=np.int32)
        self.rev_eval_ohist = np.zeros((self.ohist_size,) + simulator.model_dims, dtype=np.float32)
...
def empty_history(self):
    self.ohist.fill(self.memmory._emptyfloat)
...
def empty_eval_history(self):
    self.eval_ohist.fill(self.memmory._emptyfloat)
