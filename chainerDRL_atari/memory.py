...
class ReplayMemory(object):
  ...
  def minibatch(self, batch_size):
    ...
    if next_idx[-1] == self.memory_size:
      next_idx[-1] = 0
    ...
    return self.state[indices], 
      self.action[next_indices], 
      self.reward[next_indices], 
      next_states, 
      self.terminal[next_indices]

  def storeTuple(self, prevstate, action, reward, state, terminal=False):
